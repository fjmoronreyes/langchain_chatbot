...
class Inference:
    def __init__(self, folder_path: str, chroma_path: str = "chroma", model_name: str = "meta-llama/Meta-Llama-3-8B-Instruct"):
        ...
        self.llm = HuggingFaceEndpoint(
            repo_id=model_name, max_new_tokens=512, temperature=0.1, huggingfacehub_api_token=TOKEN, timeout=500
        )
    ...
    def parse_history(self, history: BaseChatMessageHistory) -> Tuple[str, int]:
        all_messages = " ".join([message.content for message in history.messages if isinstance(message, (HumanMessage, AIMessage))])
        tokens = all_messages.split()
        token_count = len(tokens)
        
        if token_count > 350:
            all_messages = " ".join(tokens[:350])
            token_count = 350
    ...
    def generate_with_history(self, input: str, session_id: str) -> str:
        ...
        if token_count > 300:
            logger.info("Summarizing in order to avoid losing information")
            summarized_info = self.summarizer.summarize(raw_history)
            history.messages = [HumanMessage(content=summarized_info)]
